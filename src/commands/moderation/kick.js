...
exports.run = async (bot, msg, args) => {
    if (args < 1) throw new Error('I need a user to kick');
    ...
    if (user.kickable) {
        ...
        const collector = new MessageCollector(
            msg.channel, m => m.author === msg.author, { max: 1, time: 120000 },
        );
        ...
        await collector.on('end', async (collected, reason) => {
            if (reason === 'time') msg.channel.send('The kick ended because you didn\'t provide a reason within 2 minutes.');
        });
    } else {
